...
本仓 M0/M1 **没有**独立的 Tenant 合同——租户根是 D-TENANT-DAY1 在 BR1 新建的对象。
Tenant 组因此只登记上游约束（架构与集成边界，含凭据边界），不假装存在一份 Tenant 合同。

`01_contracts_and_schemas/pinned_reuse_ledger.v0.1.yaml` 共
29 条，逐条含 `asset` / `source_path` / 四十位 `source_commit` / `file_sha256`。
...
`environment_class` 的取值域按 Founder 给定固定为 G / Z / L / S / V / P，
但六个字母各代表什么，仓内、PRD v0.3.2 与执行规划 HTML 均查无定义（已实测）。
本包**没有编造**字母含义，标 `PENDING_FOUNDER_DEFINITION` 并要求实例填写者同时写 `environment_note`。
...
三条，见回执 `open_conditions`：commit_events 的守护对象、waiver_timestamp 的取值来源、
BR0-EP01 是否仍以 A1—A4 裁决书为前置。

在三条裁定前，本包结论是 CONDITIONAL，不是 PASS。
