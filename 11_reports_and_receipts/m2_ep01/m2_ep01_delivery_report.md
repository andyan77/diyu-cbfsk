...
| P4 身份识别隔离 | 结构化能力检测 Checker 落地，扫描 270 份结构化文件零命中；8 份扫描用例（4 真实违规必命中 / 4 合规必放行）全部按声明落 |
...
## 更正记录（M2-EP04 追加，NB-M2-03）

| 位置 | 原写 | 实测 | 说明 |
|---|---|---|---|
| P4 身份识别隔离 | 扫描 **235** 份结构化文件 | 扫描 **270** 份 | Guardian 非阻塞发现 NB-M2-03。数字已改为实测值。 |

**实测方法**：在本报告所属提交 `43251b21a32fc9a3fee375675a611577babca570` 上建临时 worktree，
运行该提交自带的 `check_m2_identity_isolation.collect()`，读 `scanned_file_count`。
证据等级 `runtime_verified`。

**为什么是更正而不是追加说明**：这个数字不是判断也不是表述口径，是一个可被重新测量的事实——
原值与实测不符，就是写错了。原值保留在本表左列，未被抹去。

覆盖面数字自本包起由 `ci/checkers/check_m2_identity_isolation.py` 的
`SCAN_COVERAGE_COUNT_MISSTATED` 判据现算比对，不再靠人抄。
...
| `OI-03` | Profile 绑定仍为 PROVISIONAL |  |
| `OI-04` | STORE-A 未 provision |  |
| `OI-05` | 「D-09」无仓库锚点 |  |
| `OI-06` | founder_pinned_baseline.v0.1.yaml 的 README 指纹登记越出 §5 明文清单 |  |
...
上表第 6 项与 `OI-05` 记「**「D-09」无仓库锚点**」，并称「仓库内不存在 D-09 这一编号」。
**这句话不准确，原文保留于上，此处更正**：

`D-09` 是真实编号，出处是 `归档_v1.0/笛语跨品牌服装搭配专家内核_PRD_v1.1_Delta.docx`
第二部分「**D-09　M2 评测治理三件套　【已裁决】**」，其正文逐条给出的正是
`rater_calibration_contract` / `evaluation_sampling_design` / `benchmark_revision_protocol` 三件。

**当时为什么会判成「不存在」**：核查只在 PRD v1.2 正文与 `governance/founder_rulings/` 里 grep，
而 D-01—D-16 这批编号来自已归档的 v1.1 Delta，两处都不在检索范围内。
这是检索面不足，不是启动包写错了。

**现在怎么处置**：按 Founder 裁决 A-6，活合同、活报告与机器引用一律改为指向
**M2 冻结 Brief 第 2 节第 8/9/10 项**与三个合同 ID——理由不是「D-09 是假的」，
而是那套编号属于已归档文档的编号空间，活判据引用它会指向一份不再是产品真源的文件。
本报告与对应回执作为历史记录不改写，只追加本节。
